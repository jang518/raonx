...
대시보드는 기본적으로 팀원이 조회하는 화면입니다. 데이터 수정, 검토 상태 변경, 신뢰도 변경, 운영 데이터 반영은 `admin` 역할인 사용자만 수행합니다. 팀원은 고객 미팅 전 필요한 정보를 조회하는 `viewer` 역할로 사용합니다.
...
- 1차 개발에서는 팀원 코멘트 작성 기능을 포함하지 않습니다. 팀원은 조회 전용입니다.
...
운영 공유 전 로그인과 viewer 조회 전용 권한은 반드시 적용합니다.
