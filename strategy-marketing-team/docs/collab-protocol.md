...
| 구현 | Codex | Streamlit 앱, CSV 로딩, 검색/필터, 로그인/권한, Docker Compose, robocopy 반영 보조 파일 구현 | 작업 파일 |
...
| `output/schema_check_*.md` | CSV 스키마 검증 결과 | Codex |
...
- 역할별 결과물 문서(`final_report`, `risk_notes`, `questions`, `analysis_summary`, `schema_check`)는 `output/`에 유지.
- 새 버전이 필요하면 새 slug.
- 새 작업 시작 시 새 slug.
- 기존 인계 문서를 덮어쓰지 않는다.
...
7. `dashboard_visible=FALSE` 또는 `sensitivity_level=비공개` 데이터는 표시 정책을 반드시 확인한 뒤 구현한다.
...
| `sensitivity_level=비공개` 표시 여부 | 표시 정책이 확정되지 않으면 숨김 우선 |
| admin/viewer 권한 범위 | admin은 사용자, viewer는 팀원 조회 전용 |
| 댓글 저장 위치 | 사용자가 지정한 저장 방식 우선 |
...
- 역할별 결과물 문서(`final_report`, `risk_notes`, `questions`, `analysis_summary`, `schema_check`, 화면 캡처)는 `output/`에 유지.
- 새 버전이 필요하면 새 slug.
