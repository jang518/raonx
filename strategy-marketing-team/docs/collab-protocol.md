...
- 비밀번호, 계정 정보, Streamlit secrets, 환경변수 파일
- 담당자 개인 정보 또는 개인 연락처
...
| 5. 리스크 점검 | Claude Code | 데이터 해석, 마케팅 과잉 표현, 보안, 권한, 근거 부족 표현 점검 | `output/risk_notes_<slug>.md` |
...
현재 실제 CSV에는 `customer_id`와 `review_status`가 없다. export 생성 규칙 개선 또는 override CSV 설계 전에는 해당 컬럼이 있다고 가정하지 않는다.

목표 export 생성 규칙:

- 고객 식별용 `customer_id` 컬럼을 생성한다.
- `sensitivity_level`은 `일반/주의/비공개` 3단계로 생성한다.
- 현재 `민감/내부` 값은 legacy 값으로 본다.
- 기본 마이그레이션 제안은 `내부 → 일반`, `민감 → 주의`이며, `비공개`는 명시된 경우에만 부여한다.
...
| 화면 | 목적 |
|---|---|
| Home | 최근 고객 니즈, 자주 등장한 니즈, 검토 필요/낮은 confidence, 마케팅 후보 요약 확인 |
...
- 검토 필요 상태와 낮은 confidence를 숨기지 않는다.
...
- 1차 구현에서 인증은 사내망 내부 운영 보조 수준으로 다룬다.
- 외부 공개 수준의 강한 인증, SSO, 세밀한 권한 감사는 별도 보안 검토 전까지 범위에서 제외한다.
...
- export CSV에 포함된 신뢰도, 담당자 입력값, 노출 여부, 민감도
...
- 고객에게 실제 제안하기 전에 낮은 confidence 또는 확인 필요 상태를 숨기는 표현
...
[ ] 실제 고객 데이터, override CSV, secrets가 Git에 커밋될 가능성이 있는지 확인
...
```text
[ ] 수정 파일 확인
[ ] Git에 실데이터, override CSV, secrets가 포함되지 않았는지 확인
[ ] 검증 결과 기록
[ ] Work Board 락 해제
[ ] Handoff Log 1줄 추가
[ ] 필요한 다음 작업 명시
