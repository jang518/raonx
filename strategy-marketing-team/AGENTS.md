...
| 데이터 로딩/검증 | `output/data_validation_<slug>.md` | CSV 구조 확인 또는 데이터 검증 요청 시 |
...
| 화면/기능 검증 | `output/test_notes_<slug>.md` | 실행 테스트, 오류 점검, 화면 검증 후 |
...
- CSV 파일명이 아직 확정되지 않았으면 실제 파일명을 확인한 뒤 `docs/templates.md` 또는 `output/data_validation_<slug>.md`에 기록한다.
