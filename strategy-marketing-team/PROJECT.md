...
활성 작업(in_progress, blocked, todo)만 표시. in_progress가 파일 락(능동 작업 중), blocked는 외부 input 대기(락 없음). 산출물(app/*, data/*, scripts/*, output/*) 작업은 짧아도 행을 만들고, 메타 문서(화이트리스트) 단순 수정은 Handoff Log만. 상세 룰은 docs/collab-protocol.md §2.
