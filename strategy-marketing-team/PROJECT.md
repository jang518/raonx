...
* [2026-06-06 00:00] ChatGPT / changed: 실제 export CSV 4개 기준을 반영하고 `customer_id` 추가 필요, `dashboard_visible=TRUE/FALSE` 유지, `confidence` 중심 표시 기준을 Decisions에 정리 / verified: 현재 CSV에 `customer_id`와 `review_status`가 없고 `confidence`가 존재함을 기준으로 확인 / next: export 생성 로직에서 `customer_id` 추가 설계 필요
...
* [2026-06-06 00:00] ChatGPT / changed: `review_status`는 1차 범위에서 제외하고 `sensitivity_level`은 내부 직원용 대시보드의 표시/필터/배지 기준에서 제외 / verified: business-context.md, collab-protocol.md, AGENTS.md, CLAUDE.md, DESIGN.md, docs/templates.md와 정합성 확인 / next: Codex가 현재 CSV와 목표 export 스키마 차이를 `docs/data-schema.md` 또는 `output/analysis_summary_dashboard-foundation.md`에 정리
* [2026-06-06 00:00] ChatGPT / changed: Handoff Log에서 과거 결정과 혼동될 수 있는 `sensitivity_level 3단계`, `docs/templates.md 업데이트 실패` 이력을 제거하고 최신 기준으로 정리 / verified: Decisions 본문은 최신 기준 유지, Handoff Log는 최신 운영 기준과 일치하도록 정리 / next: WB-002 또는 WB-007 작업 착수 권장
