...
- 목표 export 생성 규칙은 `customer_id` 추가, `dashboard_visible=TRUE/FALSE` 유지
...
- 대시보드는 사내망 내부 전용이며 외부 인터넷 접속은 범위에서 제외
...
- 외부 인터넷 접속, sensitivity 변환, review_status 구현은 범위에서 제외한다.
...
| 데이터 표시 | `dashboard_visible`, `confidence` 기준을 지키는가 |
| 목표 스키마 | customer_id 추가를 실제 CSV와 구분했는가 |
...
[ ] review_status와 sensitivity_level을 1차 기능 기준에서 제외했는가
